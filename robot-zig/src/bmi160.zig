...
pub fn init() !void {
    handle = @bitCast(c.i2cOpen(1, BMI160_ADDR, 0));
    if (@as(c_int, @bitCast(handle)) < 0) {
        if (handle == c.PI_NOT_INITIALISED) {
            std.debug.print("Initialize PIGPIO library before calling bmi160.init()\n", .{});
        }
        return error.I2COpenFailed;
    }

    // Check chip ID, should be 0xD1
    const chip_id: u8 = @intCast(c.i2cReadByteData(handle, REG_CHIP_ID));
    ...
    // Send reset command
    _ = c.i2cWriteByteData(handle, REG_CMD, 0xB6);
    std.time.sleep(std.time.ns_per_ms * 100); // wait 100ms

    // Set accelerometer config: ODR 100Hz, normal mode
    _ = c.i2cWriteByteData(handle, REG_ACC_CONF, 0x2a); // 400 Hz, normal mode
    _ = c.i2cWriteByteData(handle, REG_ACC_RANGE, ACCEL_RANGE_CONFIG); // ±2g

    // Set gyro config: ODR 100Hz, normal mode
    _ = c.i2cWriteByteData(handle, REG_GYR_CONF, 0x2a); // 400 Hz, normal mode
    //_ = c.i2cWriteByteData(handle, REG_GYR_RANGE, 0x00); // ±2000°/s
    //_ = c.i2cWriteByteData(handle, REG_GYR_RANGE, 0x01); // ±1000°/s
    _ = c.i2cWriteByteData(handle, REG_GYR_RANGE, GYRO_RANGE_CONFIG); // ±500°/s
    //_ = c.i2cWriteByteData(handle, REG_GYR_RANGE, 0x03); // ±250°/s
    //_ = c.i2cWriteByteData(handle, REG_GYR_RANGE, 0x04); // ±125°/s

    std.time.sleep(std.time.ns_per_ms * 100); // wait for mode to settle

    // Enable Gyroscope
    _ = c.i2cWriteByteData(handle, REG_CMD, GYRO_NORMAL_MODE);
    ...
    while (gyro_mode != GYRO_MODE_NORMAL) {
        ...
        const pmu_status = c.i2cReadByteData(handle, REG_PMU_STATUS);
        gyro_mode = @intCast((pmu_status >> 2) & 0b11);
        std.debug.print("pmu_status = {b}, gyro_mode = {b:2}\n", .{ pmu_status, gyro_mode });
    }

    // Enable Accelerometer
    _ = c.i2cWriteByteData(handle, REG_CMD, ACCEL_NORMAL_MODE);
...
pub fn test_loop() !void {
    while (true) {
        const data = try readSensorDataNormalized(handle);
...
pub fn readSensorDataNormalized() !NormalizedSensorData {
    ...
    const result = c.i2cReadI2CBlockData(handle, REG_DATA, &buffer, buffer.len);
